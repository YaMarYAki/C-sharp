...
class GradeManager
{
    ...
    public void ViewGrades()
    {
        ...
        {
            Console.WriteLine($"{i + 1}. Предмет: {grades[i].Subject}, Оценка: {grades[i].Score}, Дата: {grades[i].Date}");
        }
    }
...
class Program
{
    static void Main()
    {
        ...
        while (true)
        {
            ...
            switch (choice)
            {
                case "1": gradeManager.AddGrade(); break;
                case "2": gradeManager.ViewGrades(); break;
                case "3": gradeManager.RemoveGrade(); break;
                case "4": return;
